...
class ArticleDiffCallBack: DiffUtil.ItemCallback<ArticleItemData>(){
    override fun areItemsTheSame(oldItem: ArticleItemData, newItem: ArticleItemData): Boolean = newItem.id==oldItem.id

    override fun areContentsTheSame(oldItem: ArticleItemData, newItem: ArticleItemData): Boolean= newItem==oldItem
}
...
class ArticleVH(override val containerView: View): RecyclerView.ViewHolder(containerView), LayoutContainer{
    fun bind(
        ...
        val avatarSize: Int=containerView.context.dpToIntPx(40)
        ...
        tv_date.text=item.date.toString().format()
